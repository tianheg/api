...
```sh
PGPASSWORD=<your_password_here> pg_dump -h <your_host_here> -U <your_user_here> -p <your_port_here> -d <your_database_name_here> | gzip > data/backup-$(date +%Y-%m-%d).sql.gz
BORG_PASSPHRASE=<your_borg_passphrase_here> borg create --stats /path/to/backup/./api_backup::'{now:%Y%m%d}' ./data
